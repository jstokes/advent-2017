...
;; --- Day 10: Knot Hash --- You come across some programs that are trying to
;; implement a software emulation of a hash based on knot-tying. The hash these
;; programs are implementing isn't very strong, but you decide to help them
;; anyway. You make a mental note to remind the Elves later not to invent their
...
;; length plus the skip size. Increase the skip size by one. The list is
;; circular; if the current position and the length try to reverse elements
;; beyond the end of the list, the operation reverses using as many extra
;; elements as it needs from the front of the list. If the current position
...
;; increases to 1. The second length, 4, selects a section which wraps: 2 1) 0
;; ([3] 4. The sublist 3 4 2 1 is reversed to form 1 2 4 3: 4 3) 0 ([1] 2. The
;; current position moves forward by the length plus the skip size, a total of
;; 5, causing it not to move because it wraps around: 4 3 0 [1] 2. The skip size
;; increases to 2. The third length, 1, selects a sublist of a single element,
;; and so reversing it has no effect. The current position moves forward by the
;; length (1) plus the skip size (2): 4 [3] 0 1 2. The skip size increases to 3.
...
;; 1 0. Finally, the current position moves forward by 8: 3 4 2 1 [0]. The skip
;; size increases to 4. In this example, the first two numbers in the list end
;; up being 3 and 4; to check the process, you can multiply them together to
;; produce 12.
...
(defn knot-hash
  ...
  (let [len-xs (count xs)]
    (reduce
      (fn [[xs pos skip] length]
        (let [[to-rev after] (split-at length (drop pos (cycle xs)))]
          [(->> (take (- len-xs length) after)
                (concat (reverse to-rev))
                (cycle)
                (drop (- len-xs pos))
                (take len-xs))
           (mod (+ pos length skip) len-xs)
           (mod (inc skip) len-xs)]))
      [xs pos skip]
      lengths)))
...
(->>
  (-> "day10input.txt" io/resource slurp str/trim (str/split #","))
  (map #(Integer/parseInt %))
  (knot-hash [(range 256) 0 0])
  (first)
  (take 2)
  (apply *))
