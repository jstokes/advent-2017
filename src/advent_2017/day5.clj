...
(defn step
  [[current-pos instructions]]
  (let [instruction (nth instructions current-pos)
        next-pos (+ current-pos instruction)]
    (if (contains? instructions next-pos)
      [next-pos (update instructions current-pos inc)]
      ::out)))


(->> [0 (->> (io/resource "day5input.txt")
             slurp
             str/split-lines
             (mapv #(Integer/valueOf %)))]
     (iterate step)
     (take-while (partial not= ::out))
     (count))
