...
;; You offer to help, but first you need to understand the structure of these towers. You
;; ask each program to yell out their name, their weight, and (if they're holding a disc) the
...
(defn parse-input
  ...
  (->> input
       (str/split-lines)
       (map #(str/split % #" -> "))
       (map (fn [[node children]]
              [(first (str/split node #" "))
               (into #{} (some-> children (str/split #", ")))]))))


(defn ->parents
  [nodes]
  (reduce
    (fn [parents [node children]]
      (reduce
        #(update %1 %2 conj node)
        parents
        children))
    (zipmap (map first nodes) (repeat #{}))
    nodes))


(->> (slurp (io/resource "day7input.txt"))
     parse-input
     ->parents
