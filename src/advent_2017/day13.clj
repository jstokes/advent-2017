...
;; --- Day 13: Packet Scanners --- You need to cross a vast firewall. The firewall
;; consists of several layers, each with a security scanner that moves back and
;; forth across the layer. To succeed, you must not be detected by a scanner.
...
;; 0: 3 1: 2 4: 4 6: 4 This means that there is a layer immediately inside the
;; firewall (with range 3), a second layer immediately after that (with range 2), a
;; third layer which begins at depth 4 (with range 4), and a fourth layer which
...
(defn tick
  [[firewall depth]]
  [(map #(update % :scan-path (partial drop 1)) firewall)
   (inc depth)])
...
(let [scanners (map
                 ...
                 (-> (io/resource "day13input.txt")
                     ...
                     str/trim
                     (str/split-lines)))
      max-depth (apply max (map first scanners))]
  (->>
    (iterate tick [(->firewall scanners) 0])
    (take-while #(<= (second %) max-depth))
    (map
      (fn [[scanners depth]]
        (if-let [scanner (first (filter #(= depth (:depth %)) scanners))]
          (if (zero? (first (:scan-path scanner)))
            (* depth (:range scanner))
            0)
          0)))
    (reduce +)))
