...
;; groups can contain other groups, a } only closes the most-recently-opened unclosed group -
...
(loop [garbage? false
       ...
       level 0
       score 0
       [c & t :as chars] (-> "day9input.txt" io/resource slurp str/trim)]
  (cond
    (empty? chars)
      score
    ignore?
      (recur garbage? false level score t)
    garbage?
      (case c
        \! (recur garbage? true level score t)
        \> (recur false ignore? level score t)
        (recur garbage? ignore? level score t))
    :else
      (case c
        \! (recur garbage? true level score t)
        \{ (recur garbage? ignore? (inc level) (+ score (inc level)) t)
        \} (recur garbage? ignore? (dec level) score t)
        \, (recur garbage? ignore? level score t)
        \< (recur true ignore? level score t))))
