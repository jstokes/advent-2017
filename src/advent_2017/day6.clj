...
;; moves to the next (by index) memory bank and inserts one of the blocks. It continues doing
;; this until it runs out of blocks; if it reaches the last memory bank, it wraps around to
;; the first one.
...
(defn redistribute-blocks
  [banks]
  (let [total-banks (count banks)
        [max-bank blocks] (most-filled banks)
        to-fill (map #(mod (+ max-bank %) total-banks)
                     (map inc (range blocks)))]
    (reduce
      (fn [banks idx] (update banks idx inc))
      (assoc banks max-bank 0)
      to-fill)))


(->> (-> "day6input.txt" io/resource slurp str/trim-newline (str/split #"\t"))
     (mapv #(Integer/parseInt %))
     (iterate redistribute-blocks)
     (reduce
       (fn [seen iteration]
         (if (contains? seen iteration)
           (reduced seen)
           (conj seen iteration)))
       #{})
     (count))
